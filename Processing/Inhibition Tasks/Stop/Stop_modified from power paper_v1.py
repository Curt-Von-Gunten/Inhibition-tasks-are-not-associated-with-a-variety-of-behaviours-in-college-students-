...
os.chdir(path)
#Data for similation.
Stop3Rem.to_csv("Stop_fromPython.txt", sep="\t", index=False) 
...
##############Removing badsubs based on accuracy##########################
StopSubLev_rem = StopSubLev[-StopSubLev['Subject'].isin(BadSubsAcc)]
##############Removing badsubs based on bad number of trials##########################
StopSubLev_rem_rem = StopSubLev_rem[-StopSubLev_rem['Subject'].isin(BadSubsTrialNumber)]

#########################Winsorizing based on SSRT. This is irrelevant for simulations#####
plt.hist(StopSubLev_rem_rem['SSRT'], bins='auto')  # arguments are passed to np.histogram
plt.title("SSRTRem")
plt.show()
...
np.std(StopSubLev_rem_rem['SSRT'])
StopSubLev_rem_rem.loc[StopSubLev_rem_rem['SSRT'] > 
              StopSubLev_rem_rem['SSRT'].mean() + (np.std(StopSubLev_rem_rem['SSRT']) * 3), 'SSRT'] = StopSubLev_rem_rem['SSRT'].mean() + (np.std(StopSubLev_rem_rem['SSRT']) * 3)
StopSubLev_rem_rem.loc[StopSubLev_rem_rem['SSRT'] < 
              StopSubLev_rem_rem['SSRT'].mean() - (np.std(StopSubLev_rem_rem['SSRT']) * 3), 'SSRT'] = StopSubLev_rem_rem['SSRT'].mean() - (np.std(StopSubLev_rem_rem['SSRT']) * 3)
...
##############Export for dissertation paper##########################
path = "C:/Users/Curt/Box Sync/Bruce Projects/Dissertation/Manuscript/For Publication/EJoP/Revision/Task Reprocessing/Stop"
os.chdir(path)
#Data for similation.
Stop3Rem.to_csv("Stop.txt", sep="\t", index=False) 
